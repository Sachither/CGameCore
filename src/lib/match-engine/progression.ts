...
async function spawnQuarterFinals(
   ...
   for (const [p1, p2] of pairs) {
      ...
      // Notify both players
      pushMatchNotification(transaction, p1, p2, circuit.players[p2]?.username || 'Opponent', mRef.id, 'QF', '24 Hours');
      pushMatchNotification(transaction, p2, p1, circuit.players[p1]?.username || 'Opponent', mRef.id, 'QF', '24 Hours');
   }

   transaction.update(circuitRef, { status: 'KNOCKOUT_Q' });
   await pushGlobalCommand(transaction, circuitRef.id, `QUARTER FINALS BEGIN! 4 RANDOM MATCHES SEEDED — 24 HOUR DEADLINE.`);
}
...
async function spawnSemiFinals(
   ...
   for (const [p1, p2] of pairs) {
      ...
      // Notify both players
         pushMatchNotification(transaction, p1, p2, circuit.players[p2]?.username || 'Opponent', mRef.id, 'SF', '24 Hours');
         pushMatchNotification(transaction, p2, p1, circuit.players[p1]?.username || 'Opponent', mRef.id, 'SF', '24 Hours');
   }
   ...
   await pushGlobalCommand(transaction, circuitRef.id, `SEMI FINALS BEGIN!${byePlayer ? ` ${circuit.players[byePlayer]?.username?.toUpperCase()} ADVANCES WITH BYE.` : ''} ${pairs.length} MATCHES SEEDED — 24 HOUR DEADLINE.`);
}
...
async function spawnFinal(
   ...
   pushMatchNotification(transaction, p1, p2, circuit.players[p2]?.username || 'Opponent', mRef.id, 'FINAL', '24 Hours');
   pushMatchNotification(transaction, p2, p1, circuit.players[p1]?.username || 'Opponent', mRef.id, 'FINAL', '24 Hours');

   transaction.update(circuitRef, { status: 'KNOCKOUT_F' });
   const byeNotice = (byePlayer && p2 === byePlayer) ? ' (BYE ADVANCE)' : '';
   await pushGlobalCommand(transaction, circuitRef.id, `GRAND FINAL! ${circuit.players[p1]?.username?.toUpperCase()} vs ${circuit.players[p2]?.username?.toUpperCase()}${byeNotice} — 24 HOUR DEADLINE.`);
}
