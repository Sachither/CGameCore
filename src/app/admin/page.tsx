...
import { getPlatformStatsAction, wipeCombatQueueAction, getAuditLogsAction } from "@/app/actions/admin-actions";
import { Gavel, Users, Swords, Activity, TrendingUp, ShieldCheck, AlertTriangle, Flame, Wallet, PiggyBank, Briefcase, Landmark, ExternalLink, RefreshCw, Eraser, ShieldAlert, Trophy, History as HistoryIcon } from "lucide-react";
...
export default function AdminOverviewPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const idToken = await user.getIdToken();
      const res = await getAuditLogsAction(idToken, 50);
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Global Audit Log · Monitor Admin Activity</p>
            </div>
          </div>
          <button 
            onClick={() => setLogsVisible(!logsVisible)}
            className="px-6 py-2 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-sm hover:bg-blue-500/20 transition-all font-black text-[10px] uppercase tracking-widest flex items-center gap-2"
          >
            {logsVisible ? "Secure Archive" : "Decrypt Logs"}
            {logsLoading && <RefreshCw className="w-3 h-3 animate-spin" />}
          </button>
        </div>

        {logsVisible && (
          <div className="max-h-[500px] overflow-y-auto custom-scrollbar border border-white/5 bg-black/40 rounded-sm p-4 space-y-2">
            {logs.length === 0 ? (
              <div className="py-20 text-center">
                <p className="text-[11px] text-gray-600 font-black uppercase tracking-[0.2em]">Intercepting Data Stream...</p>
              </div>
            ) : (
              logs.map((log) => (
                <div key={log.id} className="py-3 px-4 flex items-start gap-4 hover:bg-white/[0.02] transition-colors border-l-2 border-transparent hover:border-blue-500/50">
                   <div className="text-[9px] text-gray-600 font-mono w-[140px] shrink-0 pt-0.5">
                      {new Date(log.timestamp).toLocaleString()}
                   </div>
                   <div className="flex-1">
                      <p className="text-white text-[11px] font-black uppercase tracking-widest mb-1 leading-none">
                        <span className="text-blue-400">{log.adminUid?.slice(0,6)}...</span> EXECUTED <span className="text-accent">{log.action.replace(/_/g, ' ')}</span>
                      </p>
                      {log.targetUid && (
                        <p className="text-[9px] text-gray-500 font-bold tracking-widest leading-none mb-1">TARGET: {log.targetUid}</p>
                      )}
                      {log.details && (
                        <div className="text-[9px] text-gray-400 italic bg-white/5 p-2 rounded-sm border border-white/5 mt-1">
