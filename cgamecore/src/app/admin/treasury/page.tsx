...
export default function AdminTreasuryPage() {
  ...
  const confirmReject = async () => {
    ...
    setFeedback("");
    const idToken = await user.getIdToken();
    const result = await adminRejectWithdrawalAction(idToken, ticketToReject.id, ticketToReject.uid, ticketToReject.amountCoins);
    if (result.success) {
        ...
        <div className="bg-[#0a0a0a] border border-white/5 px-6 py-3 rounded-sm flex items-center gap-3">
           ...
           <div>
              ...
              <p className="text-white font-black italic uppercase tracking-tighter text-lg mt-0.5">
                 ₦ {tickets.reduce((acc, t) => acc + (t.fiatAmount || 0), 0).toLocaleString()}
              </p>
           </div>
        </div>
      </div>
                ...
                <div key={t.id} className="bg-[#0a0a0a] border border-white/5 hover:border-white/20 transition-all rounded-sm overflow-hidden flex flex-col md:flex-row">
                   ...
                   <div className="p-6 flex-1 flex flex-col justify-center">
                      <div className="grid grid-cols-2 gap-6">
                         <div>
                            <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Bank Name</p>
                            <p className="text-sm font-bold text-white tracking-widest uppercase">{t.bankName}</p>
                         </div>
                         <div>
                            <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Account Number</p>
                            <p className="text-2xl font-mono font-black text-white tracking-widest">{t.accountNumber}</p>
                         </div>
                         <div className="col-span-2">
                            <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Beneficiary Legal Name</p>
                     ...
                     .map((tx) => (
                       <tr key={tx.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors leading-normal text-gray-300">
                          ...
                          <td className="px-6 py-4 text-white">
                             ₦ {tx.fiatAmount?.toLocaleString() || '0'}
                          </td>
                          <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4">
          <div className="bg-surface border-2 border-green-500/30 w-full max-w-sm p-6 rounded-sm shadow-[0_0_50px_rgba(34,197,94,0.15)] flex flex-col pt-8">
             ...
             <p className="text-xs text-gray-400 text-center uppercase tracking-widest font-bold leading-relaxed mb-6">
                You are about to initiate an automatic fiat transfer of <span className="text-white">₦ {ticketToApprove.fiatAmount.toLocaleString()}</span> to <span className="text-white">{ticketToApprove.accountNumber}</span> via Paystack.
             </p>
