...
function UserModal({ user: u, adminUser, onClose, onRefresh }: {
  user: UserDoc,
  adminUser: any,
  onClose: () => void,
  onRefresh: () => void
  ...
  const handleBalanceAdjust = async () => {
    ...
    if (!amount || !balanceNote.trim()) {
      setFeedback("✗ Amount and justification are required.");
      return;
    }
    if (!adminUser) return;
    const idToken = await adminUser.getIdToken();
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="space-y-2">
              <input type="number" value={balanceAmount} onChange={(e) => setBalanceAmount(e.target.value)} placeholder="Amount (e.g., 500 or -200)" className="w-full bg-black border border-white/10 focus:border-accent text-white px-4 py-2.5 text-sm font-bold rounded-sm outline-none transition-all placeholder-gray-700" />
              <input type="text" value={balanceNote} onChange={(e) => setBalanceNote(e.target.value)} placeholder="Justification (required)" className="w-full bg-black border border-white/10 focus:border-accent text-white px-4 py-2.5 text-sm font-bold rounded-sm outline-none transition-all placeholder-gray-700" />
              <button onClick={handleBalanceAdjust} disabled={loading} className="w-full py-2.5 bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/20 text-yellow-400 text-[9px] font-black uppercase tracking-widest rounded-sm transition-all flex items-center justify-center gap-2">{loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Coins className="w-3 h-3" />} Apply Balance Change</button>
            </div>
          </div>
...
export default function AdminUsersPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {selected && (
        <UserModal user={selected} adminUser={user} onClose={() => setSelected(null)} onRefresh={() => handleSearch()} />
      )}
    </div>
  );
}
