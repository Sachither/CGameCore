...
import { submitMatchResult } from '@/lib/match-service';
...
export default function SubmitResultModal({ 
  ...
  const [claim, setClaim] = useState<'WIN' | 'LOSS' | null>(null);
             ...
             <div className="space-y-6">
               ...
               {error && (
                 <div className={`p-4 rounded-[3px] border animate-in slide-in-from-top-2 duration-300 mb-4 ${error.includes('SCORE_MISMATCH') ? 'bg-red-500/10 border-red-500/50' : 'bg-surface border-surface-border'}`}>
                    <p className="text-red-500 text-[10px] font-bold uppercase tracking-widest text-center leading-relaxed">
                       {error.includes('SCORE_MISMATCH') 
                         ? "⚠️ Tactical Mismatch: Your scoreline does not match your opponent's report. Please confirm the exact score with your opponent and resubmit." 
                         : error}
                    </p>
                 </div>
               )}
