...
export default function MatchChat({ match }: { match: Match }) {
  ...
  useEffect(() => {
    ...
    // ✅ NO orderBy — avoids the composite index requirement that breaks the listener.
    const q = query(
...
Secure the objective.`,
      ...
      (error) => {
        ...
        if (error.code === 'permission-denied') {
          console.warn("[MatchChat] Firestore permission denied — awaiting metadata sync...");
          setChatError("Syncing tactical communications link...");
        } else {
          console.error("[MatchChat] Firestore listener error:", error);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if (!user) {
       alert("AUTH ERROR: You are not logged in to Firebase on this account!");
       setChatError("Auth Required for Comms.");
       return;
    }

    if (!matchId) {
       alert("MATCH ID ERROR: The match ID is undefined!");
       return;
    }
    ...
    <div className="bg-surface border border-surface-border rounded-[5px] shadow-2xl overflow-hidden flex flex-col h-full">
      ...
      <form onSubmit={handleSend} className="bg-black border-t border-surface-border p-4 shrink-0">
         <div className="flex items-center gap-2">
            ...
            />
            <button type="submit" disabled={match.status === 'CLOSED'} className="bg-accent hover:bg-accent-hover disabled:bg-surface disabled:text-gray-500 transition-colors text-black font-black uppercase tracking-widest p-3 rounded-[3px] shrink-0 flex items-center justify-center">
