...
export default function MatchReadyDispatcher() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToUserActiveMatch(user.uid, (match) => {
      // Logic: Only alert if the match is READY or WAITING_FOR_OPPONENT (newly created)
      // and if the user is NOT already on the match page.
      if (match && (match.status === 'READY' || match.status === 'WAITING' || match.status === 'WAITING_FOR_OPPONENT')) {
